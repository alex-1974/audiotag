/++
Shared streaming ID3v2 unsynchronisation decoding.

ID3v2.2, ID3v2.3 and ID3v2.4 use the same byte-stuffing primitive for
unsynchronisation: an inserted `$00` following a physical `$FF` is removed
while exposing the logical byte stream.

This module deliberately does not decide where unsynchronisation applies.
That policy remains revision-specific:

- ID3v2.2 and ID3v2.3 apply it at tag level;
- ID3v2.4 may apply it at tag/frame level according to its own structural
  rules.

No decoded buffer is materialised. Raw source representation and absolute
physical offsets remain available.
+/
module audiotag.id3v2.common.unsync;
...
/++
One logically decoded byte together with the absolute source offset of the
physical byte that produced it.
+/
struct Id3v2DecodedByte
{
...
/++
Consumes one logical byte from an ID3v2 unsynchronised byte region.

If the physical input begins with `$FF $00`, both bytes are consumed and one
logical `$FF` is returned. Otherwise exactly one physical byte is consumed.

The caller is responsible for invoking this primitive only inside a region
where the enclosing ID3 revision says unsynchronisation is effective.

Params:
    cursor = Cursor over an already bounded physical source region.

Returns:
    The next logical byte and its physical source offset, or a structured
    end-of-span error.
