/++
Terminated ID3v2.4 text-segment parsing.

This module locates encoding-dependent string terminators in the
logical frame-data byte stream.

Unsynchronisation is reversed while searching, but the returned text
segment preserves its original physical source bytes.

No character decoding is performed here.
+/
module audiotag.id3v2.v24.text_segment;

import audiotag.core.error : ParseError, ParseErrorCode;
import audiotag.core.result : ParseResult;
import audiotag.core.span : ByteSpan;

import audiotag.id3v2.v24.data_cursor :
    Id3v24DataCursor;

import audiotag.id3v2.v24.text_encoding :
    Id3v24TextEncoding,
    usesUtf16,
    terminatorWidth;


/++
One terminated ID3v2.4 encoded text segment.

`raw` contains the physical source representation of the text only.
It excludes the terminator but retains any physical unsynchronisation
stuffing bytes.

`logicalLength` is the number of logical text bytes after
unsynchronisation decoding and before the terminator.
+/
struct Id3v24TextSegment
{
    /// Encoding used by this text segment.
    Id3v24TextEncoding encoding;

    /// Physical encoded text bytes, excluding the terminator.
    ByteSpan raw;

    /// Number of logical text bytes, excluding the terminator.
    size_t logicalLength;

    /// Physical source offset of the first terminator byte.
    size_t terminatorSourceOffset;

    /// Logical terminator width: one or two zero bytes.
    @property
    ubyte terminatorLength() const
        @safe pure nothrow @nogc
    {
        return encoding.terminatorWidth;
    }

    /++
    Constructs a logical cursor over the encoded text bytes.

    Params:
        unsynchronised = Whether unsynchronisation applies to `raw`.
    +/
    Id3v24DataCursor textCursor(bool unsynchronised) const
        @safe pure nothrow @nogc
    {
        return Id3v24DataCursor(
            raw,
            unsynchronised
        );
    }
}


/++
Consumes one terminated text segment according to `encoding`.

For ISO-8859-1 and UTF-8, the first logical `$00` terminates the
string.

For UTF-16 encodings, logical bytes are examined in two-byte code-unit
pairs from the beginning of this string. Only a complete logical
`$00 $00` pair on that alignment is accepted as the terminator.

Params:
    cursor = Logical cursor positioned at the first encoded text byte.
    encoding = ID3v2.4 text encoding controlling terminator semantics.

Returns:
    The text segment without its terminator.

Error semantics:
    Any failure leaves `cursor` unchanged.

    `patternNotFound` means no complete terminator was present.
    `invalidLength` means a UTF-16 segment ended with an incomplete
    logical code unit.
+/
ParseResult!Id3v24TextSegment takeId3v24TerminatedTextSegment(
    ref Id3v24DataCursor cursor,
    Id3v24TextEncoding encoding
)
    @safe pure nothrow @nogc
{
    auto probe = cursor;

    const startRaw =
        probe.remainingRaw;

    const startLogicalPosition =
        probe.logicalPosition;

    if (!encoding.usesUtf16)
    {
        while (!probe.empty)
        {
            auto byteResult =
                probe.takeByte();

            assert(byteResult.hasValue);

            const decoded =
                byteResult.value;

            if (decoded.value == 0x00)
            {
                const rawLength =
                    decoded.sourceOffset -
                    startRaw.sourceOffset;

                const raw =
                    startRaw.subspan(
                        0,
                        rawLength
                    );

                const logicalLength =
                    probe.logicalPosition -
                    startLogicalPosition -
                    1;

                const segment =
                    Id3v24TextSegment(
                        encoding,
                        raw,
                        logicalLength,
                        decoded.sourceOffset
                    );

                cursor = probe;

                return ParseResult!Id3v24TextSegment.success(
                    segment
                );
            }
        }

        return ParseResult!Id3v24TextSegment.failure(
            ParseError(
                ParseErrorCode.patternNotFound,
                startRaw.sourceOffset,
                1,
                probe.logicalPosition -
                    startLogicalPosition
            )
        );
    }

    while (!probe.empty)
    {
        auto firstResult =
            probe.takeByte();

        assert(firstResult.hasValue);

        const first =
            firstResult.value;

        if (probe.empty)
        {
            return ParseResult!Id3v24TextSegment.failure(
                ParseError(
                    ParseErrorCode.invalidLength,
                    first.sourceOffset,
                    2,
                    1
                )
            );
        }

        auto secondResult =
            probe.takeByte();

        assert(secondResult.hasValue);

        const second =
            secondResult.value;

        if (
            first.value == 0x00 &&
            second.value == 0x00
        )
        {
            const rawLength =
                first.sourceOffset -
                startRaw.sourceOffset;

            const raw =
                startRaw.subspan(
                    0,
                    rawLength
                );

            const logicalLength =
                probe.logicalPosition -
                startLogicalPosition -
                2;

            assert((logicalLength % 2) == 0);

            const segment =
                Id3v24TextSegment(
                    encoding,
                    raw,
                    logicalLength,
                    first.sourceOffset
                );

            cursor = probe;

            return ParseResult!Id3v24TextSegment.success(
                segment
            );
        }
    }

    return ParseResult!Id3v24TextSegment.failure(
        ParseError(
            ParseErrorCode.patternNotFound,
            startRaw.sourceOffset,
            2,
            probe.logicalPosition -
                startLogicalPosition
        )
    );
}


/// Latin-1 terminates at the first logical zero byte.
unittest
{
    const ubyte[] bytes =
        ['A', 'B', 'C',
         0x00,
         0x55];

    auto cursor =
        Id3v24DataCursor(
            ByteSpan(bytes, 100),
            false
        );

    auto result =
        cursor.takeId3v24TerminatedTextSegment(
            Id3v24TextEncoding.latin1
        );

    assert(result.hasValue);

    const segment = result.value;

    assert(segment.encoding == Id3v24TextEncoding.latin1);
    assert(segment.raw.sourceOffset == 100);
    assert(segment.raw.data == ['A', 'B', 'C']);
    assert(segment.logicalLength == 3);
    assert(segment.terminatorSourceOffset == 103);
    assert(segment.terminatorLength == 1);

    assert(cursor.logicalPosition == 4);
    assert(cursor.physicalPosition == 4);
    assert(cursor.absoluteOffset == 104);
}


/// UTF-8 uses the same one-byte terminator semantics.
unittest
{
    const ubyte[] bytes =
        [0xC3, 0x84,
         0x00,
         0x55];

    auto cursor =
        Id3v24DataCursor(
            ByteSpan(bytes, 200),
            false
        );

    auto result =
        cursor.takeId3v24TerminatedTextSegment(
            Id3v24TextEncoding.utf8
        );

    assert(result.hasValue);
    assert(result.value.raw.data == [0xC3, 0x84]);
    assert(result.value.logicalLength == 2);
    assert(result.value.terminatorSourceOffset == 202);

    assert(cursor.absoluteOffset == 203);
}


/// Empty one-byte strings consume only their terminator.
unittest
{
    const ubyte[] bytes =
        [0x00, 0x55];

    auto cursor =
        Id3v24DataCursor(
            ByteSpan(bytes, 300),
            false
        );

    auto result =
        cursor.takeId3v24TerminatedTextSegment(
            Id3v24TextEncoding.utf8
        );

    assert(result.hasValue);
    assert(result.value.raw.empty);
    assert(result.value.raw.sourceOffset == 300);
    assert(result.value.logicalLength == 0);
    assert(result.value.terminatorSourceOffset == 300);

    assert(cursor.absoluteOffset == 301);
}


/// UTF-16 terminators are recognised only on code-unit alignment.
unittest
{
    const ubyte[] bytes =
        [
            0x41, 0x00,
            0x00, 0x42,
            0x00, 0x00,
            0x55
        ];

    auto cursor =
        Id3v24DataCursor(
            ByteSpan(bytes, 400),
            false
        );

    auto result =
        cursor.takeId3v24TerminatedTextSegment(
            Id3v24TextEncoding.utf16
        );

    assert(result.hasValue);

    const segment = result.value;

    // The adjacent zeros at logical offsets 1 and 2 cross a
    // code-unit boundary and must not terminate the string.
    assert(
        segment.raw.data ==
        [0x41, 0x00, 0x00, 0x42]
    );

    assert(segment.logicalLength == 4);
    assert(segment.terminatorSourceOffset == 404);
    assert(segment.terminatorLength == 2);

    assert(cursor.absoluteOffset == 406);
}


/// UTF-16BE uses the same aligned structural terminator rule.
unittest
{
    const ubyte[] bytes =
        [
            0x00, 0x41,
            0x00, 0x42,
            0x00, 0x00
        ];

    auto cursor =
        Id3v24DataCursor(
            ByteSpan(bytes, 500),
            false
        );

    auto result =
        cursor.takeId3v24TerminatedTextSegment(
            Id3v24TextEncoding.utf16be
        );

    assert(result.hasValue);
    assert(result.value.raw.length == 4);
    assert(result.value.logicalLength == 4);
    assert(result.value.terminatorSourceOffset == 504);

    assert(cursor.empty);
}


/// Empty UTF-16 strings consist only of a two-byte terminator.
unittest
{
    const ubyte[] bytes =
        [0x00, 0x00, 0x55];

    auto cursor =
        Id3v24DataCursor(
            ByteSpan(bytes, 600),
            false
        );

    auto result =
        cursor.takeId3v24TerminatedTextSegment(
            Id3v24TextEncoding.utf16be
        );

    assert(result.hasValue);
    assert(result.value.raw.empty);
    assert(result.value.logicalLength == 0);
    assert(result.value.terminatorSourceOffset == 600);

    assert(cursor.absoluteOffset == 602);
}


/// Latin-1 segmentation preserves physical unsynchronisation bytes.
unittest
{
    // Logical bytes are $FF followed by the terminating $00.
    // Physical representation contains the stuffing zero.
    const ubyte[] bytes =
        [0xFF, 0x00,
         0x00,
         0x55];

    auto cursor =
        Id3v24DataCursor(
            ByteSpan(bytes, 700),
            true
        );

    auto result =
        cursor.takeId3v24TerminatedTextSegment(
            Id3v24TextEncoding.latin1
        );

    assert(result.hasValue);

    const segment = result.value;

    assert(segment.logicalLength == 1);

    // The physical stuffing byte remains part of the raw text span.
    assert(segment.raw.data == [0xFF, 0x00]);
    assert(segment.raw.length == 2);

    assert(segment.terminatorSourceOffset == 702);

    assert(cursor.logicalPosition == 2);
    assert(cursor.physicalPosition == 3);
    assert(cursor.absoluteOffset == 703);
}


/// UTF-16 alignment is based on logical rather than physical bytes.
unittest
{
    // Logical stream:
    //
    //   00 FF | 00 00
    //
    // The first pair is text and the second pair is the terminator.
    // Unsynchronisation inserts one physical zero after FF.
    const ubyte[] bytes =
        [
            0x00, 0xFF, 0x00,
            0x00, 0x00,
            0x55
        ];

    auto cursor =
        Id3v24DataCursor(
            ByteSpan(bytes, 800),
            true
        );

    auto result =
        cursor.takeId3v24TerminatedTextSegment(
            Id3v24TextEncoding.utf16be
        );

    assert(result.hasValue);

    const segment = result.value;

    assert(segment.logicalLength == 2);

    // Physical raw text includes the stuffing zero after FF.
    assert(
        segment.raw.data ==
        [0x00, 0xFF, 0x00]
    );

    assert(segment.terminatorSourceOffset == 803);

    assert(cursor.logicalPosition == 4);
    assert(cursor.physicalPosition == 5);
    assert(cursor.absoluteOffset == 805);
}


/// Missing one-byte terminators fail atomically.
unittest
{
    const ubyte[] bytes =
        ['A', 'B', 'C'];

    auto cursor =
        Id3v24DataCursor(
            ByteSpan(bytes, 900),
            false
        );

    auto result =
        cursor.takeId3v24TerminatedTextSegment(
            Id3v24TextEncoding.utf8
        );

    assert(result.hasError);
    assert(result.error.code == ParseErrorCode.patternNotFound);
    assert(result.error.offset == 900);
    assert(result.error.requested == 1);
    assert(result.error.available == 3);

    assert(cursor.logicalPosition == 0);
    assert(cursor.physicalPosition == 0);
    assert(cursor.absoluteOffset == 900);
}


/// Missing aligned UTF-16 terminators fail atomically.
unittest
{
    const ubyte[] bytes =
        [
            0x00, 0x41,
            0x00, 0x42
        ];

    auto cursor =
        Id3v24DataCursor(
            ByteSpan(bytes, 1000),
            false
        );

    auto result =
        cursor.takeId3v24TerminatedTextSegment(
            Id3v24TextEncoding.utf16be
        );

    assert(result.hasError);
    assert(result.error.code == ParseErrorCode.patternNotFound);
    assert(result.error.offset == 1000);
    assert(result.error.requested == 2);
    assert(result.error.available == 4);

    assert(cursor.logicalPosition == 0);
    assert(cursor.physicalPosition == 0);
}


/// An incomplete UTF-16 code unit is reported separately.
unittest
{
    const ubyte[] bytes =
        [
            0x00, 0x41,
            0x55
        ];

    auto cursor =
        Id3v24DataCursor(
            ByteSpan(bytes, 1100),
            false
        );

    auto result =
        cursor.takeId3v24TerminatedTextSegment(
            Id3v24TextEncoding.utf16be
        );

    assert(result.hasError);
    assert(result.error.code == ParseErrorCode.invalidLength);
    assert(result.error.offset == 1102);
    assert(result.error.requested == 2);
    assert(result.error.available == 1);

    assert(cursor.logicalPosition == 0);
    assert(cursor.physicalPosition == 0);
}


/// Consecutive terminated strings can be consumed from one cursor.
unittest
{
    const ubyte[] bytes =
        [
            'A', 0x00,
            'B', 0x00,
            0x55
        ];

    auto cursor =
        Id3v24DataCursor(
            ByteSpan(bytes, 1200),
            false
        );

    auto first =
        cursor.takeId3v24TerminatedTextSegment(
            Id3v24TextEncoding.latin1
        );

    auto second =
        cursor.takeId3v24TerminatedTextSegment(
            Id3v24TextEncoding.latin1
        );

    assert(first.hasValue);
    assert(first.value.raw.data == ['A']);

    assert(second.hasValue);
    assert(second.value.raw.data == ['B']);

    assert(cursor.absoluteOffset == 1204);
    assert(cursor.remainingRaw.data == [0x55]);
}
