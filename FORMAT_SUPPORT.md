...
| MPEG Audio / MP3 | ID3v2.3 | Core | Core | Active bounded parser, canonical mapping including `TRCK`/`TPOS` track/disc positions, `TCON` genres and aggregated `TYER`/`TDAT`/`TIME` recording timestamps, complete tag codec writer with whole-tag unsynchronisation and extended-header CRC support, plus POSIX path-based leading-ID3v2 insertion/replacement/removal; MPEG audio remainder is preserved opaquely rather than validated |
